...
resource "aws_iam_role_policy" "jenkins_dr_control" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # ⚠️ POC용 광범위 권한 포함. Phase 6에서 최소권한 리팩터링 필요.
      # ============================================================
      ...
      # [3] Terraform State Backend (terraform init 필수)
      {
        ...
        Resource = [
          "arn:aws:s3:::3team-tfstate-dr-01",
          "arn:aws:s3:::3team-tfstate-dr-01/*"
        ]
      },

      # [4] Terraform State Lock
      # NOTE: aws_region 변수는 root/dr 에만 있고 module/compute 에는 와이어링 안 되어 있어
      #       리전을 ap-northeast-2 로 하드코딩. 멀티 리전 시 변수화 필요.
      {
      ...
      # ⚠️ POC 용도. Phase 6에서 AssumeRole 패턴으로 최소권한 리팩터링 필요.
      {
